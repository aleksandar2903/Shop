                    ...
                    <div class="card-header">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('transactions.type', ['type' => 'expense']) }}" class="btn btn-sm btn-primary">{{ __('Back to List')}}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('transactions.update', $transaction) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('payment_method_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-method">{{ __('Categories')}}</label>
                                ...
                                <div class="form-group{{ $errors->has('amount') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="number" step=".01" name="amount" id="input-amount" class="form-control form-control-alternative" placeholder="{{__('Amount')}}" value="{{ old('amount', abs($transaction->amount)) }}" min="0" required>
